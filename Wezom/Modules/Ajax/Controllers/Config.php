<?php
    ...
    class Config extends \Wezom\Modules\Ajax {
        ...
        public function testEmailAction() {
            ...
            if (!$title) {
                $this->error('Заполните заголовок письма');
            }
            $body = trim(Arr::get($this->post, 'body', ''));
            if (!$body) {
                $this->error('Заполните тело письма');
            }
            ...
            if(Email::send($title, $body, $email)){
                $this->success('Письмо успешно отправлено');
            }
            $this->error('Произошла ошибка при отправке письма. Проверьте настройки почты');
        }
    }